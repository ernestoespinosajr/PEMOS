...
interface UseElectionRealtimeOptions {
  /**
...
interface ElectionDashboardData {
...
/**
 * Extended dashboard data hook for the election-night monitoring view.
 *
 * Builds on the same realtime infrastructure as useDashboardData but adds:
 *   - Candidate vote comparison data (by-candidate endpoint)
 *   - Precinct acta progress data (precinct-progress endpoint)
 *   - All 6 data streams updated on Realtime events
 *
 * Uses the existing useRealtimeSubscription and usePollingFallback hooks
 * from ftr-006. Does NOT duplicate their logic.
 */
export function useElectionRealtime({
  periodoId,
  isActivePeriod,
  filters,
}: UseElectionRealtimeOptions): ElectionDashboardData {
  ...
  const [candidates, setCandidates] = useState<CandidateVoteData[]>([]);
  ...
  const fetchAllData = useCallback(async () => {
    ...
    try {
      ...
      const [
        ...
        fetch(
          `/api/electoral/dashboard/summary?${summaryParams.toString()}`
        ),
        ...
        fetch(
          `/api/electoral/dashboard/turnout${
            filters.recinto_id ? `?recinto_id=${filters.recinto_id}` : ''
          }`
        ),
        fetch(
          `/api/electoral/dashboard/by-candidate?${summaryParams.toString()}`
        ),
        fetch(
          `/api/electoral/dashboard/precinct-progress?${summaryParams.toString()}`
        ),
      ]);
      ...
      if (!summaryRes.ok) {
        ...
        setError(
          candidateJson.error ?? 'Error al cargar votos por candidato'
        );
      } else if (!progressRes.ok) {
        setError(
          progressJson.error ?? 'Error al cargar progreso de recintos'
        );
      }
    } catch (err) {
      console.error('Error fetching election dashboard data:', err);
